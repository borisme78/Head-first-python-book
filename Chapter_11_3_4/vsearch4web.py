...
def log_request(req: "flask_request", res: str) -> None:
    ...
    """
    # Обгортаємо запит до БД у try/except для запобігання перериванню основного флоу
    try:
        with UseDatabase(app.config["dbconfig"]) as cursor:
            ...
            # Виконуємо запит з плейсхолдерами для безпеки від SQL-ін'єкцій
            cursor.execute(
                _SQL,
                (
                    req.form["phrase"],
    ...
    # Якщо логування виконалось неправильно, виводимо помилку, але не ламаємо основний код
...
def do_search() -> "html":
    ...
    try:
        # Створюємо новий потік для асинхронного логування запиту
        # Це дозволяє не чекати завершення логування і негайно повернути результат користувачу
        t = Thread(target=log_request, args=(request, result))
        # Запускаємо потік на фоні (в іншому потоці виконання)
        t.start()
    # Если створення потока або його запуск завершилось помилкою
    except Exception as err:
        print("***** Logging failed with this error:", str(err))
